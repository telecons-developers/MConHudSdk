...
object ConnectionManager {
    ...
    fun registerListener(listener: ConnectionEventListener) {
        ...
        Logger.i("$TAG Added listener $listener, ${listeners.size} listeners total")
    }
    ...
    fun unregisterListener(listener: ConnectionEventListener) {
        ...
        toRemove?.let {
            listeners.remove(it)
            Logger.i("$TAG Removed listener ${it.get()}, ${listeners.size} listeners total")
        }
    }
    ...
    fun connect(device: BluetoothDevice?, context: Context) : Boolean {
        ...
        return if (device.isConnected()) {
            Logger.e("$TAG Already connected to ${device.address}!")
            false
        } else {
    ...
    fun teardownConnection(device: BluetoothDevice?) {
        ...
        if (device.isConnected()) {
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot teardown connection!")
        }
    }
    ...
    fun readCharacteristic(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() && characteristic.isReadable()) {
            ...
            Logger.e("$TAG Attempting to read ${characteristic.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            Logger.e("$TAG Not connected to ${device.address}, cannot perform characteristic read")
        }
    }
    ...
    fun writeCharacteristic(
        ...
        val writeType = when {
            ...
            characteristic.isWritableWithoutResponse() -> {
                ...
                Logger.e("$TAG Characteristic ${characteristic.uuid} cannot be written to")
                return
            }
        }
        if (device.isConnected()) {
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot perform characteristic write")
        }
    }
    ...
    fun readDescriptor(device: BluetoothDevice, descriptor: BluetoothGattDescriptor) {
        if (device.isConnected() && descriptor.isReadable()) {
            ...
            Logger.e("$TAG Attempting to read ${descriptor.uuid} that isn't readable!")
        } else if (!device.isConnected()) {
            Logger.e("$TAG Not connected to ${device.address}, cannot perform descriptor read")
        }
    }
    ...
    fun writeDescriptor(
        ...
        if (device.isConnected() && (descriptor.isWritable() || descriptor.isCccd())) {
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot perform descriptor write")
        } else if (!descriptor.isWritable() && !descriptor.isCccd()) {
            Logger.e("$TAG Descriptor ${descriptor.uuid} cannot be written to")
        }
    }
    ...
    fun enableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot enable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            Logger.e("$TAG Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun disableNotifications(device: BluetoothDevice, characteristic: BluetoothGattCharacteristic) {
        if (device.isConnected() &&
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot disable notifications")
        } else if (!characteristic.isIndicatable() && !characteristic.isNotifiable()) {
            Logger.e("$TAG Characteristic ${characteristic.uuid} doesn't support notifications/indications")
        }
    }
    ...
    fun requestMtu(device: BluetoothDevice, mtu: Int) {
        if (device.isConnected()) {
            ...
            Logger.e("$TAG Not connected to ${device.address}, cannot request MTU update!")
        }
    }
    ...
    private fun signalEndOfOperation() {
        Logger.i("$TAG End of $pendingOperation")
    ...
    private fun doNextOperation() {
        if (pendingOperation != null) {
            Logger.e("$TAG doNextOperation() called when an operation is pending! Aborting.")
            return
        }

        val operation = operationQueue.poll() ?: run {
            Logger.v("Operation queue empty, returning")
            return
        }
        ...
        if (operation is Connect) {
            with(operation) {
                Logger.e("$TAG Connecting to ${device.address}")
        ...
        val gatt = deviceGattMap[operation.device]
            ?: this@ConnectionManager.run {
                Logger.e("$TAG Not connected to ${operation.device.address}! Aborting $operation operation.")
        ...
        when (operation) {
            is Disconnect -> with(operation) {
                Logger.e("$TAG Disconnecting from ${device.address}")
                gatt.close()
            ...
            is CharacteristicWrite -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    Logger.e("$TAG Cannot find $characteristicUuid to write to")
                    signalEndOfOperation()
                }
            }
            is CharacteristicRead -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    Logger.e("$TAG Cannot find $characteristicUuid to read from")
                    signalEndOfOperation()
                }
            }
            is DescriptorWrite -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    Logger.e("$TAG Cannot find $descriptorUuid to write to")
                    signalEndOfOperation()
                }
            }
            is DescriptorRead -> with(operation) {
                gatt.findDescriptor(descriptorUuid)?.let { descriptor ->
                    ...
                    Logger.e("$TAG Cannot find $descriptorUuid to read from")
                    signalEndOfOperation()
                }
            }
                ...
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    characteristic.getDescriptor(cccdUuid)?.let { cccDescriptor ->
                        if (!gatt.setCharacteristicNotification(characteristic, true)) {
                            Logger.e("$TAG setCharacteristicNotification failed for ${characteristic.uuid}")
                            signalEndOfOperation()
                            return
                        }
                        ...
                        Logger.e("$TAG ${characteristic.uuid} doesn't contain the CCC descriptor!")
                        signalEndOfOperation()
                    }
                } ?: this@ConnectionManager.run {
                    Logger.e("$TAG Cannot find $characteristicUuid! Failed to enable notifications.")
                    signalEndOfOperation()
                }
            }
            is DisableNotifications -> with(operation) {
                gatt.findCharacteristic(characteristicUuid)?.let { characteristic ->
                    ...
                    Logger.e("$TAG Cannot find $characteristicUuid! Failed to disable notifications.")
                    signalEndOfOperation()
                }
            }
    ...
    private val callback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            if (status == BluetoothGatt.GATT_SUCCESS) {
                if (newState == BluetoothProfile.STATE_CONNECTED) {
                    Logger.e("$TAG onConnectionStateChange: connected to $deviceAddress")
                    deviceGattMap[gatt.device] = gatt
                    Handler(Looper.getMainLooper()).post {
                        gatt.discoverServices()
                    }
                } else if (newState == BluetoothProfile.STATE_DISCONNECTED) {
                    Logger.e("$TAG onConnectionStateChange: disconnected from $deviceAddress")
                    teardownConnection(gatt.device)
                }
            } else {
                Logger.e("$TAG onConnectionStateChange: status $status encountered for $deviceAddress!")
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt, status: Int) {
            with(gatt) {
                if (status == BluetoothGatt.GATT_SUCCESS) {
                    Logger.e("$TAG Discovered ${services.size} services for ${device.address}.")
                    printGattTable()
                    requestMtu(device, GATT_MAX_MTU_SIZE)
                    listeners.forEach { it.get()?.onConnectionSetupComplete?.invoke(this) }
                } else {
                    Logger.e("$TAG Service discovery failed due to status $status")
                    teardownConnection(gatt.device)
                }
            }
        ...
        override fun onMtuChanged(gatt: BluetoothGatt, mtu: Int, status: Int) {
            Logger.e("$TAG ATT MTU changed to $mtu, success: ${status == BluetoothGatt.GATT_SUCCESS}")
            ...
            with(characteristic) {
                when (status) {
                    ...
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        Logger.e("$TAG Read not permitted for $uuid!")
                    }
                    else -> {
                        Logger.e("$TAG Characteristic read failed for $uuid, error: $status")
                    }
                }
            }
            ...
            with(characteristic) {
                when (status) {
                    ...
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        Logger.e("$TAG Write not permitted for $uuid!")
                    }
                    else -> {
                        Logger.e("$TAG Characteristic write failed for $uuid, error: $status")
                    }
                }
            }
            ...
            with(descriptor) {
                when (status) {
                    ...
                    BluetoothGatt.GATT_READ_NOT_PERMITTED -> {
                        ...
                        Logger.e("$TAG Descriptor read failed for $uuid, error: $status")
                    }
                }
            }
            ...
            with(descriptor) {
                when (status) {
                    BluetoothGatt.GATT_SUCCESS -> {
                        ...
                        listeners.forEach { it.get()?.onCommunicationReady?.invoke(gatt.device) }
                    }
                    BluetoothGatt.GATT_WRITE_NOT_PERMITTED -> {
                        Logger.e("$TAG Write not permitted for $uuid!")
                    }
                    else -> {
                        Logger.e("$TAG Descriptor write failed for $uuid, error: $status")
                    }
                }
            }
        ...
        private fun onCccdWrite(
            ...
            when {
                notificationsEnabled -> {
                    Logger.e("$TAG Notifications or indications ENABLED on $charUuid")
                    listeners.forEach {
                ...
                notificationsDisabled -> {
                    Logger.e("$TAG Notifications or indications DISABLED on $charUuid")
                    listeners.forEach {
                    ...
                    Logger.e("$TAG Unexpected value ${value.toHexString()} on CCCD of $charUuid")
                }
            }
        }
    }
        ...
        override fun onReceive(context: Context, intent: Intent) {
            with(intent) {
                if (action == BluetoothDevice.ACTION_BOND_STATE_CHANGED) {
                    ...
                    Logger.e("$TAG ${device?.address} bond state changed | $bondTransition")
                }
            }
        }
